/**
 * MeetIntegration.gs — Google Meet + Bộ Email (v6.1.0)
 * v6.1.0: mọi dữ liệu người dùng nhập đi qua esc_() trước khi vào HTML (trước đây HV đặt tên chứa HTML
 *         là chèn được link/ảnh vào email gửi tutor và admin); gửi qua sendMail_ (không ném lỗi);
 *         email cảnh báo số buổi trả true/false để chỉ đánh cờ khi gửi được; email huỷ tìm tutor theo ID
...
function sendFailureNotifications(studentEmail,studentName,studentId,failedSlots,quota,successCount){
  ...
  if(CONFIG.EMAIL.SEND_FAILURE_STUDENT&&studentEmail){
    var blocks='';for(var reason in byReason){var items=byReason[reason],li='';for(var k=0;k<items.length;k++)li+='<li>'+formatDate_(items[k].date)+' · '+esc_(items[k].timeSlot)+'</li>';blocks+='<div style="border:1px solid #f3d6d3;background:#FDECEA;border-radius:10px;padding:14px 18px;margin:0 0 12px"><div style="font-size:13px;font-weight:700;color:#8a1f1a;margin-bottom:6px">'+esc_(reason)+'</div><ul style="margin:0;padding-left:18px;color:#6b3330;font-size:13px;line-height:1.8">'+li+'</ul></div>';}
    var body=emailBanner_(successCount>0?'Một phần đăng ký chưa thành công':'Đăng ký chưa thành công','','#FDECEA','#8a1f1a')+'<p style="font-size:15px;color:#1b2a4a">Chào <b>'+(nameH||'bạn')+'</b>,</p>'+blocks+(quota&&quota.found?balanceBox_(quota.total,quota.used,quota.remaining):'');
    sendMail_({to:studentEmail,subject:(successCount>0?'Một phần lịch chưa xác nhận':'Đăng ký chưa thành công')+' | '+CONFIG.SCHOOL_NAME,htmlBody:emailWrapper_(body,'#E53935',''),replyTo:getReplyToEmail_(),name:CONFIG.SCHOOL_NAME});}
  if(CONFIG.EMAIL.NOTIFY_ADMIN_ON_FAILURE){var adminList='';for(var r2 in byReason){var arr=byReason[r2],lis='';for(var m=0;m<arr.length;m++)lis+='<li>'+formatDate_(arr[m].date)+' · '+esc_(arr[m].timeSlot)+'</li>';adminList+='<div style="margin:0 0 10px"><b style="color:#8a1f1a">'+esc_(r2)+'</b><ul style="margin:4px 0;padding-left:18px;font-size:13px;color:#4a5568">'+lis+'</ul></div>';}
    var adminBody=emailBanner_('Booking thất bại',failedSlots.length+' slot.','#FFF4E5','#8a4a00')+'<div style="border:1px solid #e4e9f0;border-radius:10px;padding:16px 20px"><table style="width:100%;border-collapse:collapse">'+infoRow_('Học viên',nameH)+infoRow_('Email',emailH)+infoRow_('Thành công',successCount+' buổi')+infoRow_('Thất bại',failedSlots.length+' buổi')+'</table></div>'+adminList;
    sendMail_({to:CONFIG.ADMIN_EMAIL,subject:'[Admin] Booking thất bại — '+(studentName||studentEmail),htmlBody:emailWrapper_(adminBody,'#FB8C00',''),name:CONFIG.SCHOOL_NAME});}
}

// ── 3. SẮP HẾT BUỔI ──
function sendLowBalanceEmail_(quota,remaining,bookings){
  if(!quota.email)return false;
  var body=emailBanner_('Bạn còn '+remaining+' buổi học','Gia hạn sớm để không gián đoạn.','#FFF4E5','#8a4a00')+'<p style="font-size:15px;color:#1b2a4a">Chào <b>'+esc_(quota.name)+'</b>,</p><p style="font-size:14px;color:#4a5568;line-height:1.7">Gói học của bạn sắp dùng hết.</p>'+balanceBox_(quota.total,quota.total-remaining,remaining)+ctaButton_('Liên hệ gia hạn','mailto:'+getReplyToEmail_()+'?subject='+encodeURIComponent('Gia hạn — '+quota.email),'#FB8C00');
  return sendMail_({to:quota.email,subject:'Bạn còn '+remaining+' buổi học | '+CONFIG.SCHOOL_NAME,htmlBody:emailWrapper_(body,'#FB8C00',''),replyTo:getReplyToEmail_(),name:CONFIG.SCHOOL_NAME});
}

// ── 4. HẾT BUỔI ──
function sendQuotaExhaustedEmail_(quota,consumed,bookings){
  if(!quota.email)return false;
  var body=emailBanner_('Gói học đã dùng hết','Gia hạn để tiếp tục.','#FDECEA','#8a1f1a')+'<p style="font-size:15px;color:#1b2a4a">Chào <b>'+esc_(quota.name)+'</b>,</p><p style="font-size:14px;color:#4a5568;line-height:1.7">Gói học của bạn đã hết. Form đăng ký sẽ tạm từ chối buổi mới.</p>'+balanceBox_(quota.total,quota.total,0)+ctaButton_('Gia hạn gói học','mailto:'+getReplyToEmail_()+'?subject='+encodeURIComponent('Gia hạn — '+quota.email),'#2F5496');
  return sendMail_({to:quota.email,cc:CONFIG.QUOTA.ADMIN_CC_ON_EXHAUSTED?CONFIG.ADMIN_EMAIL:'',subject:'Gói học đã dùng hết | '+CONFIG.SCHOOL_NAME,htmlBody:emailWrapper_(body,'#E53935',''),replyTo:getReplyToEmail_(),name:CONFIG.SCHOOL_NAME});
}

// ── 5. HUỶ BUỔI ──
function sendCancelNotification(bookingRow){
  if(!CONFIG.EMAIL.SEND_CANCEL_NOTIFICATION)return;
  var C=CONFIG.BOOKING_COLS,bookingId=bookingRow[C.BOOKING_ID-1],studentName=bookingRow[C.STUDENT_NAME-1],studentEmail=String(bookingRow[C.STUDENT_EMAIL-1]||'').trim(),tutorId=bookingRow[C.TUTOR_ID-1],tutorName=bookingRow[C.TUTOR_NAME-1],date=bookingRow[C.DATE-1],timeSlot=bookingRow[C.TIME_SLOT-1];
  var nameH=esc_(studentName),tutorH=esc_(tutorName),slotH=esc_(timeSlot);
  var dateStr=(date instanceof Date)?formatDate_(date):esc_(date),dayStr=(date instanceof Date)?getDayName_(date):'';
  var quota=getStudentQuotaByEmail_(studentEmail);
  if(studentEmail){var body=emailBanner_('Buổi học đã được huỷ','Buổi đã hoàn lại.','#FFF8E1','#7a5c00')+'<p style="font-size:15px;color:#1b2a4a">Chào <b>'+nameH+'</b>,</p><div style="border:1px solid #e4e9f0;border-radius:10px;padding:16px 20px"><table style="width:100%;border-collapse:collapse">'+infoRow_('Ngày',dateStr+' · '+dayStr)+infoRow_('Khung giờ',slotH)+infoRow_('Giảng viên',tutorH||'—')+infoRow_('Mã',esc_(bookingId))+'</table></div>'+(quota.found?balanceBox_(quota.total,quota.used,quota.remaining):'');sendMail_({to:studentEmail,subject:'Đã huỷ buổi học '+dateStr+' | '+CONFIG.SCHOOL_NAME,htmlBody:emailWrapper_(body,'#FBC02D',''),replyTo:getReplyToEmail_(),name:CONFIG.SCHOOL_NAME});}
  var tutor=findTutor_(tutorId,tutorName),tutorEmail=tutor?tutor.email:'';
  if(tutorEmail){var tBody=emailBanner_('Buổi dạy đã huỷ','Khung giờ đã giải phóng.','#FFF8E1','#7a5c00')+'<p style="font-size:15px;color:#1b2a4a">Chào <b>'+tutorH+'</b>,</p><div style="border:1px solid #e4e9f0;border-radius:10px;padding:16px 20px"><table style="width:100%;border-collapse:collapse">'+infoRow_('Học viên',nameH)+infoRow_('Ngày',dateStr)+infoRow_('Khung giờ',slotH)+'</table></div>';sendMail_({to:tutorEmail,subject:'Huỷ buổi dạy '+dateStr+' | '+CONFIG.SCHOOL_NAME,htmlBody:emailWrapper_(tBody,'#FBC02D',''),replyTo:getReplyToEmail_(),name:CONFIG.SCHOOL_NAME});}
}
...
function sendWelcomeEmail_(email,name,studentId,packageName,sessions){
  ...
  var nameH=esc_(name),emailH=esc_(email);
  var bookingUrl=bkId?'https://docs.google.com/forms/d/'+bkId+'/viewform':'';
  var body=emailBanner_('Chào mừng bạn đến với '+CONFIG.SCHOOL_NAME,'Tài khoản đã kích hoạt.','#EDF7ED','#1B5E20')+'<p style="font-size:15px;color:#1b2a4a">Chào <b>'+nameH+'</b>,</p><p style="font-size:14px;color:#4a5568;line-height:1.7">Thanh toán đã được xác nhận.</p><div style="border:1px solid #e4e9f0;border-radius:10px;padding:16px 20px;margin:0 0 20px"><table style="width:100%;border-collapse:collapse">'+infoRow_('Mã học viên',esc_(studentId))+infoRow_('Email',emailH)+infoRow_('Gói học',esc_(packageName))+infoRow_('Số buổi',sessions+' buổi')+'</table></div>'+balanceBox_(sessions,0,sessions)+'<div style="border:1px solid #e4e9f0;border-radius:10px;padding:16px 20px;margin:0 0 20px"><div style="font-size:13px;font-weight:700;color:#1b2a4a;margin-bottom:10px">Cách đặt lịch</div><ol style="margin:0;padding-left:18px;color:#4a5568;font-size:13.5px;line-height:1.9"><li>Mở form đặt lịch</li><li>Đặt bằng <b>đúng email này</b> ('+emailH+')</li><li>Chọn khung giờ phù hợp</li><li>Nhận email xác nhận kèm link Meet</li></ol></div>'+(bookingUrl?ctaButton_('Đặt lịch học ngay',bookingUrl,'#2F5496'):'')+'<div style="background:#FFF4E5;border-radius:10px;padding:14px 18px"><div style="font-size:12.5px;color:#8a4a00;line-height:1.8"><b>Quan trọng:</b> luôn đặt lịch bằng đúng email đã đăng ký.</div></div>';
  return sendMail_({to:email,subject:'Chào mừng đến với '+CONFIG.SCHOOL_NAME+' — Tài khoản đã kích hoạt',htmlBody:emailWrapper_(body,'#2E7D32','Tài khoản kích hoạt với '+sessions+' buổi.'),replyTo:getReplyToEmail_(),name:CONFIG.SCHOOL_NAME});
}
